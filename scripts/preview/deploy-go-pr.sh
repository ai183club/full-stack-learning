...
tag="Project=profile-learning,Environment=pr-$pr_number,ManagedBy=github-actions"
...
if [ -z "$secret_arn" ] || [ "$secret_arn" = None ]; then
 secret_arn=$(aws secretsmanager create-secret --name "$database_secret_name" --region "$AWS_REGION" --generate-secret-string "SecretStringTemplate={\"username\":\"$database_role\"},GenerateStringKey=password,PasswordLength=40,ExcludePunctuation=true" --tags "$tag" --query ARN --output text)
fi
...
if [ -z "$api_id" ] || [ "$api_id" = None ]; then api_id=$(aws apigatewayv2 create-api --region "$AWS_REGION" --name "$api_gateway_name" --protocol-type HTTP --cors-configuration "AllowOrigins=https://$web_host,AllowMethods=GET,POST,PATCH,OPTIONS,AllowHeaders=content-type,authorization" --tags "$tag" --query ApiId --output text); fi
